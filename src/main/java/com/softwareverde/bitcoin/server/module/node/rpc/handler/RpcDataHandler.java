...
public class RpcDataHandler implements NodeRpcHandler.DataHandler {
    ...
    @Override
    public void invalidateBlock(final Sha256Hash blockHash) {
        try (final FullNodeDatabaseManager databaseManager = _databaseManagerFactory.newDatabaseManager()) {
            final BlockHeaderDatabaseManager blockHeaderDatabaseManager = databaseManager.getBlockHeaderDatabaseManager();
            blockHeaderDatabaseManager.markBlockAsInvalid(blockHash, BlockHeaderDatabaseManager.INVALID_PROCESS_THRESHOLD);

            // TODO: Fix UTXO set...
            // TODO: Fix unconfirmed transactions...
            // Use BlockProcessor:switchHeadBlock (?)
        }
        catch (final Exception exception) {
            Logger.debug(exception);
        }
